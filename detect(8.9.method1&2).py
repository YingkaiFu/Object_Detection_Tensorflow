...
def judge_box(box, i, count):
    ...
    for j in range(count):
        ...
        if (box[i][2] > box[j][0] and box[i][3] > box[j][1]
            ...
            flag.append(j)
            print('the object is overlapped with other items')
    return flag
...
def main(itemId=[], positionx=[], positiony=[], Theta=[]):
    ...
    name = '/home/yingkai/Data/8.9/test2.jpg'
    image_path = name
    ...
    for i in range(output_dict['detection_boxes'].shape[0]):
        if output_dict['detection_scores'] is None or output_dict['detection_scores'][i] > 0.8:
            ...
            edges = cv2.Canny(crop_image, 104, 33, apertureSize=3)
            ...
            if ifsquare(item) == 2:
                ...
                # 重新设置往左和往右的长度和比例
                if lby > 0.75 * (maxy - miny):
                    lf = round(0.7 * lby)
                    rf = round((maxy - miny - rby) * 0.3)
                    rbflag = 1
                elif lby < 0.75 * (maxy - miny):
                    lf = round(0.3 * lby)
                    rf = round((maxy - miny - rby) * 0.7)
                    lbflag = 1
                ...
                if b1 > b2:
                    ...
                    newmaxy = by + itemright * ((bx - minx) / b1)
                    newmaxx = bx - itemright * ((by - miny) / b1)
                    cv2.line(perspective, (by, bx), (round(newmaxy), round(newmaxx)), 255)
                    cv2.line(img_perspective, (by, bx), (round(newmaxy), round(newmaxx)), 255)
                    theta = math.asin((by - miny) / b1) * 180 / math.pi
                    print(newmaxx, newmaxy)
                else:
                    print('长边在右')
                    itemright, itemleft = (getitem(getid(item)))
                    newminy = by - itemleft * (bx - maxx) / b2
                    newminx = bx - itemleft * (maxy - by) / b2
                    cv2.line(perspective, (by, bx), (round(newminy), round(newminx)), 255)
                    cv2.line(img_perspective, (by, bx), (round(newminy), round(newminx)), 255)
                    theta = math.asin((maxy - by) / b2) * 180 / math.pi
                ...
                Theta.append('TODO' + '  ')
                cv2.imshow('crop', crop_image)
                cv2.imshow('pers', img_perspective)
                cv2.waitKey()
